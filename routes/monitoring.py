...
def domain_metrics(domain_id):
    ...
    bandwidth_data = [m.bandwidth_used / (1024 * 1024) for m in metrics]  # Convert to MB
    response_time_data = [m.avg_response_time for m in metrics]
...
def api_domain_metrics(domain_id):
    ...
    data = {
        ...
        'bandwidth': [m.bandwidth_used for m in metrics],
        'response_time': [m.avg_response_time for m in metrics],
