...
def create():
    ...
    if request.method == 'POST':
        ...
        domain = Domain(
            ...
            ffpanel_dns='dnspod'  # По умолчанию используем 'dnspod' для FFPanel
        )
        
        db.session.add(domain)
        db.session.commit()
        
        # Синхронизируем с FFPanel если нужно
        if ffpanel_enabled:
            sync_result = DomainManager.sync_domain_with_ffpanel(domain.id)
            if not sync_result['success']:
                flash(f'Внимание: Домен создан в системе, но не добавлен в FFPanel: {sync_result["message"]}', 'warning')
...
def edit(domain_id):
    ...
    if request.method == 'POST':
        ...
        # Если FFPanel включен, но не указан DNS, используем 'dnspod' по умолчанию
        if ffpanel_enabled and not domain.ffpanel_dns:
            domain.ffpanel_dns = 'dnspod'
        ...
        # Синхронизируем с FFPanel если нужно 
        # (если опция включена и ранее не синхронизировался)
        if ffpanel_enabled:
            sync_result = DomainManager.sync_domain_with_ffpanel(domain.id)
            if not sync_result['success']:
                flash(f'Внимание: Домен обновлен в системе, но не добавлен в FFPanel: {sync_result["message"]}', 'warning')
...
def check_ns(domain_id):
    ...
    try:
        ...
        # Выполняем проверку в отдельном блоке с ускоренной обработкой для веб-запросов
        result = DomainManager.check_domain_ns_status(domain_id, web_request=True)
...
def check_all_ns():
    ...
    try:
        ...
        # Создаем специальную версию проверки NS для веб-запросов (без задержек)
        try:
            # Добавим веб-оптимизированную проверку
            results = DomainManager.check_all_domains_ns_status(web_request=True)
            logger.info(f"Результаты проверки всех NS-записей через веб-интерфейс: {results}")
...
def api_check_nameservers(domain_name):
    ...
    try:
        ...
        try:
            ...
            # Используем ускоренную проверку без задержек для веб-запросов
            nameservers = DomainManager.check_nameservers(domain_name, web_request=True)
...
def ffpanel(domain_id):
    ...
    if request.method == 'POST':
        ...
        if action == 'sync':
            ...
            domain.ffpanel_dns = request.form.get('ffpanel_dns', 'dnspod')  # По умолчанию используем 'dnspod'
            db.session.commit()
