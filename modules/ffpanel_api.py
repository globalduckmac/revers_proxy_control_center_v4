...
class FFPanelAPI:
    ...
    def add_site(self, domain, ip_path, port="80", port_out="80", dns="dnspod"):
        """
        ...
        Args:
            ...
            dns: DNS-адрес (по умолчанию dnspod). Важно: параметр не может быть пустым!
            
        Returns:
            dict: Словарь с результатом операции: {'success': bool, 'id': int или None, 'message': str}
        """
        # Проверяем параметр dns - он не должен быть пустым
        if not dns:
            dns = "dnspod"  # Устанавливаем dnspod по умолчанию, если параметр пустой
        try:
            headers = self._get_headers()
            headers.pop('Content-Type', None)  # Убираем Content-Type для multipart/form-data
            
            # Проверяем, что значение dns не пустое и допустимое
            if not dns or dns not in ["dnspod", "custom"]:
                dns = "dnspod"  # Устанавливаем dnspod как значение по умолчанию
