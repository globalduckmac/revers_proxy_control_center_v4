...
class DomainManager:
    ...
    @staticmethod
    def check_nameservers(domain_name, max_attempts=3, retry_delay=5, web_request=False):
        """
        ...
        Args:
            ...
            web_request: Флаг, указывающий, что запрос выполняется из веб-интерфейса (без задержек)
        ...
        # Для веб-запросов используем только 1 попытку без задержки
        if web_request:
            max_attempts = 1
        ...
        for attempt in range(1, max_attempts + 1):
            try:
                ...
                resolver.timeout = 5 if web_request else 10  # Уменьшаем таймаут для веб-запросов
                ...
                if attempt < max_attempts and not web_request:
                    logger.warning(f"{error_msg}. Retrying in {retry_delay} seconds...")
                    time.sleep(retry_delay)
                else:
                    # Это была последняя попытка или веб-запрос
                    logger.error(f"{error_msg}. Giving up after {attempt} attempts.")
                    break
    ...
    @staticmethod
    def check_domain_ns_status(domain_id, web_request=False):
        """
        Проверяет соответствие NS-записей домена с ожидаемыми значениями
        
        Args:
            domain_id: ID домена для проверки
            web_request: Флаг, указывающий, что запрос выполняется из веб-интерфейса
        ...
        try:
            ...
            actual_ns = DomainManager.check_nameservers(domain.name, web_request=web_request)
            ...
            # Логируем для отладки все NS-записи
            logger.info(f"Domain {masked_domain_name} checking NS. Expected: {expected_ns_lowercase}, Actual: {actual_ns_lowercase}")
            
            for expected_ns in expected_ns_lowercase:
                # Проверяем, содержится ли ожидаемый NS как часть любого из реальных NS
                matching_records = [actual for actual in actual_ns_lowercase if expected_ns in actual]
                if matching_records:
                    is_special_match = True
                    logger.info(f"Special match for domain {masked_domain_name}: expected NS '{expected_ns}' found as substring in actual NS records: {matching_records}")
                    break
                
                # Или наоборот, если актуальный NS является частью ожидаемого
                reverse_matching = [actual for actual in actual_ns_lowercase if actual in expected_ns]
                if reverse_matching:
                    is_special_match = True
                    logger.info(f"Special match for domain {masked_domain_name}: actual NS {reverse_matching} contains expected NS '{expected_ns}'")
                    break
            
            # Стандартная проверка - точное совпадение
            all_expected_found = all(ns in actual_ns_lowercase for ns in expected_ns_lowercase)
            
            # Подробное логирование решения
            if all_expected_found:
                logger.info(f"Domain {masked_domain_name} NS exact match: ALL expected NS found in actual NS list")
            elif is_special_match:
                logger.info(f"Domain {masked_domain_name} NS partial match: Substring matching applied successfully")
            
            # Принятие решения о статусе
            if all_expected_found or is_special_match:
                previous_status = domain.ns_status
                domain.ns_status = 'ok'
                if previous_status != 'ok':
                    logger.info(f"Domain {masked_domain_name} NS status changed: {previous_status} -> ok")
                else:
                    logger.info(f"Domain {masked_domain_name} NS check: OK. No status change.")
            else:
            ...
            # Добавляем запись в журнал DomainLog о результате проверки
            from models import DomainLog
            
            # Определяем статус для лога
            log_status = 'success' if domain.ns_status == 'ok' else 'warning'
            
            # Формируем сообщение для лога
            if domain.ns_status == 'ok':
                log_message = f"NS check passed. Expected: {domain.expected_nameservers}, Actual: {domain.actual_nameservers}"
            else:
                log_message = f"NS check failed. Expected: {domain.expected_nameservers}, Actual: {domain.actual_nameservers}"
    ...
    @staticmethod
    def check_all_domains_ns_status(web_request=False):
        """
        Проверяет NS-статус для всех доменов, у которых указаны ожидаемые NS
        
        Args:
            web_request: Флаг, указывающий, что запрос выполняется из веб-интерфейса
                         (без задержек между проверками)
        ...
        try:
            ...
            for domain in domains:
                ...
                try:
                    ...
                    if DomainManager.check_domain_ns_status(domain_id, web_request=web_request):
                        results['ok'] += 1
        ...
        try:
            ...
            if domain.ffpanel_id:
                ...
                result = api.add_site(
                    ...
                    dns=domain.ffpanel_dns or 'dnspod'  # По умолчанию используем 'dnspod'
                )
