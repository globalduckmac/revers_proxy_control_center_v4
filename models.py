...
class Server(db.Model):
    ...
    is_active = db.Column(db.Boolean, default=True)  # Флаг активности сервера
    last_status = db.Column(db.String(20), default='pending')  # online, offline, error
    ...
    # Метрики Glances
    glances_cpu = db.Column(db.Float, nullable=True)  # Загрузка CPU в процентах
    glances_memory = db.Column(db.Float, nullable=True)  # Использование памяти в процентах
    glances_disk = db.Column(db.Text, nullable=True)  # JSON с информацией о дисках
    glances_network = db.Column(db.Text, nullable=True)  # JSON с информацией о сетевых интерфейсах
    glances_load = db.Column(db.String(64), nullable=True)  # Load average (1, 5, 15 минут)
    glances_uptime = db.Column(db.BigInteger, nullable=True)  # Uptime в секундах
...
class Domain(db.Model):
    ...
    is_active = db.Column(db.Boolean, default=True)  # Флаг активности домена
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    updated_at = db.Column(db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
    ...
    # Предыдущий статус NS для отслеживания изменений
    previous_ns_status = db.Column(db.String(20), nullable=True)
    ...
    @property
    def ns_status_changed(self):
        """
        Определяет, изменился ли статус NS записей домена.
        
        Returns:
            bool: True если статус изменился, False в противном случае
        """
        return self.previous_ns_status is not None and self.previous_ns_status != self.ns_status
...
class ServerMetric(db.Model):
    ...
    memory_total = db.Column(db.BigInteger, nullable=True)  # Общий объем памяти в байтах
    memory_used = db.Column(db.BigInteger, nullable=True)   # Использованный объем памяти в байтах
    disk_usage = db.Column(db.Float, nullable=True)
    disk_info = db.Column(db.Text, nullable=True)  # JSON с информацией о дисках
    load_average = db.Column(db.String(100), nullable=True)  # Увеличено с 30 до 100 символов
    timestamp = db.Column(db.DateTime, default=datetime.utcnow, index=True)
    collection_method = db.Column(db.String(20), default='glances_api', nullable=True)  # 'ssh' или 'glances_api'
...
class SystemSetting(db.Model):
    ...
    def get_value(key, default=None):
        ...
        Returns:
            str или None: Значение настройки или значение по умолчанию
        """
        setting = SystemSetting.query.filter_by(key=key).first()
        if not setting:
            return default
            
        # Проверяем, нужно ли расшифровать значение
    ...
    @staticmethod
    def set_value(key, value, description=None, is_encrypted=False):
        """
        ...
        Args:
            ...
            description (str, optional): Описание настройки
            is_encrypted (bool): Нужно ли зашифровать значение
            
        Returns:
            SystemSetting: Объект настройки
        """
        # Если нужно зашифровать и значение не None
...
class ExternalServer(db.Model):
    """Внешние серверы, мониторинг которых осуществляется только через Glances API."""
    id = db.Column(db.Integer, primary_key=True)
    ...
    # Отношение с метриками
    metrics = db.relationship('ExternalServerMetric', backref='external_server', lazy=True, cascade='all, delete-orphan')
    ...
    def get_latest_metric(self, metric_type, metric_name):
        """
        Получает последнюю метрику указанного типа
        
        Args:
            metric_type: Тип метрики (cpu, memory, disk, network)
            metric_name: Имя метрики (total, percent, detail)
            
        Returns:
            ExternalServerMetric: Объект метрики или None
        """
        from sqlalchemy import desc
        return ExternalServerMetric.query.filter_by(
            external_server_id=self.id,
            metric_type=metric_type,
            metric_name=metric_name
        ).order_by(desc(ExternalServerMetric.created_at)).first()
    
    def get_metric_history(self, metric_type, metric_name, limit=24):
        """
        Получает историю метрик указанного типа
        ...
        return ExternalServerMetric.query.filter_by(
            ...
            metric_type=metric_type,
            metric_name=metric_name
        ).order_by(desc(ExternalServerMetric.created_at)).limit(limit).all()


class ExternalServerMetric(db.Model):
    """Метрики мониторинга внешних серверов."""
    id = db.Column(db.Integer, primary_key=True)
    ...
    metric_name = db.Column(db.String(50), nullable=False)  # total, percent, detail
    metric_value = db.Column(db.Text, nullable=False)  # JSON или числовое значение в виде строки
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
